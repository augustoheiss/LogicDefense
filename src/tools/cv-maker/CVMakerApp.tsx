...
export const CVMakerApp: React.FC = () => {
  // Sidebar mode: 'chat' (AI Assistant) or 'editor' (Raw YAML Editor)
  ...
  // When AI generates new versions, switch active version
  ...
  // Reset to default
  const handleReset = () => {
    if (!window.confirm('Deseja restaurar o modelo padrão de exemplo? Suas edições atuais serão substituídas.')) return
    setCvVersions(null)
    ...
    <div className="cv-maker-app">
      ...
      <div className="cv-app-header cv-no-print">
        ...
        <div className="cv-app-controls">
          <button className="cv-btn-secondary" onClick={handleReset} title="Restaurar modelo padrão">
            🔄 Resetar Modelo
          </button>
          <button
      ...
      <div className="cv-split-layout">
        {/* Left Column: Sidebar (Chat / YAML Editor) */}
