  ...
  return (
    <div className="cv-modal-backdrop" onClick={onClose}>
      <div className="cv-modal-card" style={{ maxWidth: '750px' }} onClick={e => e.stopPropagation()}>
        ...
        {/* ── Sub Navigation (2 Abas Unificadas) ── */}
        <div style={{ display: 'flex', gap: '0.5rem', borderBottom: '1px solid rgba(255, 255, 255, 0.08)', paddingBottom: '0.5rem' }}>
          ...
          >
            🤖 Hub de Agentes & Terminal (OpenAPI, cURL, Python)
          </button>
        </div>

        <div className="cv-modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxHeight: '74vh', overflowY: 'auto' }}>
          {errorMsg && (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', fontSize: '0.82rem', color: '#cbd5e1', lineHeight: 1.6 }}>
              ...
              <div style={{ background: 'rgba(16, 185, 129, 0.08)', padding: '0.85rem', borderRadius: '6px', border: '1px solid rgba(16, 185, 129, 0.25)' }}>
                ...
                <ul style={{ margin: '0.4rem 0 0 1.2rem', padding: 0 }}>
                  <li><strong>POST /api/v1/cv/compile</strong>: O Agente envia os 5 YAMLs gerados e recebe o Super Dashboard HTML interativo com fotos, 5 temas e botão nativo de impressão A4.</li>
...
3. Salve o arquivo HTML retornado e abra no navegador para imprimir em PDF A4!`}
              ...
              {/* 4. Guia de Terminal & Código (cURL & Python) */}
...
{`curl -X POST "https://ocorrencias-pdf-writer.onrender.com/api/v1/cv/compile" \\
  ...
  --output "dashboard_curriculos.html"`}
                ...
                <div>
                  <span style={{ fontSize: '0.78rem', color: '#94a3b8' }}><strong>B. Renderizar 1 YAML em HTML Estilizado & PDF (Python):</strong></span>
...
res = requests.post(
    ...
    json={"yaml_content": open("cv.yaml").read(), "theme": "executive"}
)
...
# Pronto! O arquivo HTML abre no navegador com botão nativo de impressão A4.`}
                ...
                <div>
                  <span style={{ fontSize: '0.78rem', color: '#94a3b8' }}><strong>C. Auto-provisionamento de Chave Temporária via Terminal:</strong></span>
